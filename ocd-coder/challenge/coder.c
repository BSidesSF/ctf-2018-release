...
int setup_sandbox();
void sort_shellcode(void *execp, size_t size);
...
int main(int argc, char **argv) {
  ...
  print_msg("And I mean everything -- even our shellcode storage service.");
  print_msg("For storage efficiency, we put all your shellcode into order.");
  ...
  print_msg("Sorting your shellcode.");
  sort_shellcode(sc_buf, sc_len);
...
void sort_shellcode(void *execp, size_t size) {
  ...
  size &= (~3);
  // Begin sorting
  qsort(execp, size/sizeof(uint32_t), sizeof(uint32_t), compare_ints);
}
...
int setup_sandbox() {
  ...
  // Filter open
  rv |= seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(open), 1,
